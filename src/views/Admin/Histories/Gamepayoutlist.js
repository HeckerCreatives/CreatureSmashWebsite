...
const GamePayoutList = () => {
    ...
    useEffect(() => {
        setIsLoading(true)
        fetch(`${process.env.REACT_APP_API_URL}/payout/getpayoutlistadmin?page=${page - 1}&limit=10&type=gamebalance`,{
            method: 'GET',
            credentials: 'include',
            headers: {
                    ...
                    <MDBTableBody>
                    {
                        list.length !== 0 ?
                            ...
                            <tr key={i} className="text-center">
                                ...
                                <td>{data.status}</td>
                                <td>
                                    <MDBBtn className="m-1" size="sm" color="info" onClick={() => handlePayout(data.id, "done")}>
                                        Process
                                    </MDBBtn>
